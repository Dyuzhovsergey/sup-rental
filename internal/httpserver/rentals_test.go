...
func TestRentalWizardShowsClientAndAvailableModels(t *testing.T) {
	...
	for _, want := range []string{"15.08.2026 10:08 — 11:38", "1 час 30 мин"} {
		if !strings.Contains(equipmentResponse.Body.String(), want) {
...
func TestRentalWizardShowsReviewWithCompositionAndTotals(t *testing.T) {
	...
	for _, want := range []string{
		"Шаг 4. Итог", "Анна Петрова", "15.08.2026 10:08 — 11:38", "1 час 30 мин",
...
func TestRentalSlotsLabelUsesFullHourWord(t *testing.T) {
	tests := []struct {
		slots int
		want  string
	}{
		...
		{slots: 4, want: "2 часа"},
		{slots: 10, want: "5 часов"},
		{slots: 42, want: "21 час"},
		{slots: 44, want: "22 часа"},
	}
	for _, tt := range tests {
		if got := rentalSlotsLabel(tt.slots); got != tt.want {
			t.Errorf("rentalSlotsLabel(%d) = %q, want %q", tt.slots, got, tt.want)
		}
	}
}
...
func TestCreateConfirmedRentalRedirectsToList(t *testing.T) {
	...
	rentals := &rentalServiceStub{
		create: func(_ context.Context, actor user.User, clientID int64, interval rental.Interval, selections []rental.ModelSelection) (rental.Rental, error) {
			...
			return rental.Restore(24, clientID, interval, rental.StatusConfirmed, nil, nil, []rental.Item{{
...
func TestRentalsListAndDetail(t *testing.T) {
	...
	stored, err := rental.Restore(24, 18, interval, rental.StatusConfirmed, nil, nil, []rental.Item{item})
	if err != nil {
...
func TestRentalMutationsRequireOperatorAndCSRF(t *testing.T) {
	...
	for _, request := range []*http.Request{
		...
		rentalRequest(http.MethodGet, "/rentals/24/complete", nil),
		rentalRequest(http.MethodPost, "/rentals/24/complete", url.Values{"csrf_token": {"csrf-token"}}),
		...
		rentalRequest(http.MethodGet, "/rentals/bulk/complete?rental_id=24", nil),
		rentalRequest(http.MethodPost, "/rentals/bulk/complete", url.Values{"csrf_token": {"csrf-token"}, "rental_id": {"24"}}),
	} {
		response := httptest.NewRecorder()
	...
	for _, token := range []string{"", "wrong"} {
		for _, path := range []string{"/rentals", "/rentals/24/cancel", "/rentals/24/complete", "/rentals/bulk/issue", "/rentals/bulk/cancel", "/rentals/bulk/complete"} {
			response := httptest.NewRecorder()
...
func TestRentalsListSeparatesStatusesAndShowsConfirmedActions(t *testing.T) {
	...
	for _, want := range []string{
		"Подтверждённый клиент", "Активный клиент", "Завершённый клиент",
		"Длительность", "1 час 30 мин", "main-content--wide",
		"rental-period-cell", "rental-duration-cell", "rental-composition-cell",
		`status-chip status-chip--available"><span class="status-dot" aria-hidden="true"></span>Завершена`,
		`href="/rentals/24/issue"`, `href="/rentals/24/cancel"`, "Выдать оборудование",
		`href="/rentals/25/complete"`, "Принять возврат",
	...
	for _, unwanted := range []string{`href="/rentals/25/issue"`, `href="/rentals/26/cancel"`, `href="/rentals/24/complete"`} {
	...
	for _, unwanted := range []string{"data-rental-bulk-form", `name="rental_id"`, "Выдать выбранные", "Отменить выбранные", "Принять возврат выбранных"} {
...
func TestRentalBulkConfirmationAndRedirects(t *testing.T) {
	...
	confirmedForID := func(id int64) rental.Rental {
		value, err := rental.Restore(id, 18, interval, rental.StatusConfirmed, nil, nil, []rental.Item{{
	...
	activeForID := func(id int64) rental.Rental {
		issuedAt := interval.Start().Add(-time.Minute)
		value, err := rental.Restore(id, 18, interval, rental.StatusActive, &issuedAt, nil, []rental.Item{{
			EquipmentID: id + 100, InventoryNumber: fmt.Sprintf("SUP-TOURING-%d", id), Kind: equipment.KindSUPBoard,
			ModelCode: "TOURING", HourlyRateKopecks: 100_000,
		}})
		if err != nil {
			t.Fatalf("Restore active(%d) error = %v", id, err)
		}
		return value
	}
	var issuedIDs, cancelledIDs, completedIDs []int64
	rentals := &rentalServiceStub{
		get: func(_ context.Context, id int64) (rental.Rental, error) {
			if id >= 26 {
				return activeForID(id), nil
			}
			return confirmedForID(id), nil
		},
		...
		completeMany: func(_ context.Context, actor user.User, ids []int64) ([]rental.Rental, error) {
			if actor.Role != user.RoleOperator {
				t.Errorf("CompleteMany() actor = %+v", actor)
			}
	...
	for _, tt := range []struct {
		...
		{path: "/rentals/bulk/issue?rental_id=24&rental_id=25", ids: []string{"№24", "№25"}},
		{path: "/rentals/bulk/cancel?rental_id=24&rental_id=25", ids: []string{"№24", "№25"}},
		{path: "/rentals/bulk/complete?rental_id=26&rental_id=27", ids: []string{"№26", "№27"}},
	} {
		response := httptest.NewRecorder()
		handler.ServeHTTP(response, rentalRequest(http.MethodGet, tt.path, nil))
		if response.Code != http.StatusOK {
	...
	completeForm := url.Values{"csrf_token": {"csrf-token"}, "rental_id": {"26", "27"}}
	completeResponse := httptest.NewRecorder()
	handler.ServeHTTP(completeResponse, rentalRequest(http.MethodPost, "/rentals/bulk/complete", completeForm))
	if completeResponse.Code != http.StatusSeeOther || completeResponse.Header().Get("Location") != "/rentals?bulk_completed=2" {
		t.Fatalf("bulk complete = %d Location %q", completeResponse.Code, completeResponse.Header().Get("Location"))
	}
	if fmt.Sprint(issuedIDs) != "[24 25]" || fmt.Sprint(cancelledIDs) != "[24 25]" || fmt.Sprint(completedIDs) != "[26 27]" {
		t.Fatalf("issued = %v cancelled = %v completed = %v", issuedIDs, cancelledIDs, completedIDs)
	}
}
...
func TestRentalsListShowsBulkSuccessNotices(t *testing.T) {
	for query, want := range map[string]string{
		...
		"bulk_completed=2": "Завершены 2 аренды.",
	} {
		response := httptest.NewRecorder()
		newRentalTestHandler(t, user.RoleOperator, &rentalServiceStub{}, rentalClientsStub()).ServeHTTP(
...
func TestRentalsListShowsIssuedNotice(t *testing.T) {
	interval := rentalHTTPInterval(t)
	issuedAt := time.Date(2026, time.August, 15, 10, 8, 0, 0, moscowTimeZone)
	active, err := rental.Restore(24, 18, interval, rental.StatusActive, &issuedAt, nil, []rental.Item{{
		EquipmentID: 94, InventoryNumber: "SUP-TOURING-1", Kind: equipment.KindSUPBoard,
		ModelCode: "TOURING", HourlyRateKopecks: 100_000,
	}})
	if err != nil {
...
func TestRentalCompletionConfirmationAndRedirect(t *testing.T) {
	interval := rentalHTTPInterval(t)
	issuedAt := time.Date(2026, time.August, 15, 10, 2, 0, 0, moscowTimeZone)
	active, err := rental.Restore(24, 18, interval, rental.StatusActive, &issuedAt, nil, []rental.Item{{
		EquipmentID: 94, InventoryNumber: "SUP-TOURING-1", Kind: equipment.KindSUPBoard,
		ModelCode: "TOURING", HourlyRateKopecks: 100_000,
	}})
	...
	page := httptest.NewRecorder()
	handler.ServeHTTP(page, rentalRequest(http.MethodGet, "/rentals/24/complete", nil))
	if page.Code != http.StatusOK {
		t.Fatalf("complete page status = %d body %q", page.Code, page.Body.String())
	}
	for _, want := range []string{
		...
		if !strings.Contains(page.Body.String(), want) {
			t.Errorf("complete page does not contain %q", want)
		}
	}
	...
	detail := httptest.NewRecorder()
	newRentalTestHandler(t, user.RoleOperator, &rentalServiceStub{
		get: func(context.Context, int64) (rental.Rental, error) { return completed, nil },
...
func TestRentalCompletionPageErrorsAreMappedSafely(t *testing.T) {
	interval := rentalHTTPInterval(t)
	confirmed, err := rental.Restore(24, 18, interval, rental.StatusConfirmed, nil, nil, []rental.Item{{
		EquipmentID: 94, InventoryNumber: "SUP-TOURING-1", Kind: equipment.KindSUPBoard,
		ModelCode: "TOURING", HourlyRateKopecks: 100_000,
	...
	tests := []struct {
		...
		{name: "invalid id", path: "/rentals/not-a-number/complete", wantStatus: http.StatusNotFound, wantText: "404 page not found"},
		{name: "missing", path: "/rentals/24/complete", get: func(context.Context, int64) (rental.Rental, error) {
			return rental.Rental{}, rental.ErrRentalNotFound
		}, wantStatus: http.StatusNotFound, wantText: "404 page not found"},
		{name: "wrong status", path: "/rentals/24/complete", get: func(context.Context, int64) (rental.Rental, error) {
			return confirmed, nil
		}, wantStatus: http.StatusConflict, wantText: "только по активной аренде"},
		{name: "internal", path: "/rentals/24/complete", get: func(context.Context, int64) (rental.Rental, error) {
...
func TestRentalCompletionErrorsAreMappedSafely(t *testing.T) {
	tests := []struct {
		name       string
		err        error
		wantStatus int
...
func TestRentalCancellationPageErrorsAreMappedSafely(t *testing.T) {
	interval := rentalHTTPInterval(t)
	cancelled, err := rental.Restore(24, 18, interval, rental.StatusCancelled, nil, nil, []rental.Item{{
		EquipmentID: 94, InventoryNumber: "SUP-TOURING-1", Kind: equipment.KindSUPBoard,
		ModelCode: "TOURING", HourlyRateKopecks: 100_000,
	}})
	if err != nil {
...
func newRentalTestHandler(t *testing.T, role user.Role, rentals rentalService, clients clientService) http.Handler {
	...
	handler, err := NewHandler(
		...
		rentals, &adminDashboardServiceStub{}, CookieSettings{}, ClientIPSettings{},
	)
	if err != nil {
