...
Интерфейс адаптируется от смартфонов шириной `320px` до desktop viewport
`1440px` и выше в соответствии с `docs/ui-style-guide.md`. Mobile navigation,
локальная прокрутка широких таблиц, touch targets, keyboard navigation, видимый
focus, доступные labels, контраст и browser zoom остаются обязательными.
...
Responsive-рекомендации skill применяй в границах утверждённых breakpoint и
паттернов из `docs/ui-style-guide.md`, не создавая отдельный mobile frontend.
...
Routine-команды `go build ./...`, `docker compose build`,
`docker compose up -d`, `docker compose up --build -d` и
`docker compose ps -a` заранее разрешены локальными правилами проекта. Запускай
их в указанном виде и не запрашивай повторное подтверждение пользователя.

Не оборачивай `go build ./...` без необходимости в `env`, `set -a`,
`source .env` или составную shell-команду: такая форма перестаёт совпадать с
узким allow-правилом. Сборка Go не требует конфигурации приложения из `.env`.
Обычные тесты запускай без `TEST_DATABASE_URL`; integration-тест подключай
только к явно выбранной актуальной тестовой или Compose PostgreSQL, не подставляя
автоматически случайную локальную базу из `.env`.
